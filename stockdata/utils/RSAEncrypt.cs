...
namespace stockdata.utils
{
    class RSAEncrypt
    {
        private static string RSAPublicKeyFileName = AppDomain.CurrentDomain.BaseDirectory + Path.DirectorySeparatorChar + "publickey.rsa";
        private static string _RSAPublicKey = null;

        /// <summary>
        /// 암호화에 사용할 RSA Public key
        /// </summary>
        public static string RSAPublicKey
        {
            get
            {
                if (_RSAPublicKey == null)
                {
                    ...
                    {
                        _RSAPublicKey = File.ReadAllText(RSAPublicKeyFileName);
                    }
                    else
                    {
                        ...
                        if (client.doWorkDialog())
                        {
                            ...
                            File.WriteAllText(RSAPublicKeyFileName, _RSAPublicKey);
                        }
                    }
                }
        ...
        {
            RSACryptoServiceProvider rsa = DecodeX509PublicKey(DecodeOpenSSLPublicKey(RSAPublicKey));

            return Convert.ToBase64String(rsa.Encrypt(plain, false));
        }
